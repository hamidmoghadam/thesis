...
#    selected_filenames = []
#    for filename in glob.glob(os.sep.join([source_dir_data, '0*_*'])):
#        if threshold <= int(os.path.basename(filename).split('_')[0]):
#            selected_filenames.append(filename)
#    return selected_filenames


def sample_tweets(input_dir, features):
    tweets_sampled = {}
    for author in glob.glob(os.sep.join([input_dir, '0*_*'])):
...
def classify(x_train, y_train, x_test, y_test, work_dir):
    logging.debug(''.join(['\t\tSize of training feature vector: ', str(x_train.shape[0]), ' lines and ', str(x_train.shape[1]), ' columns.']))
    ...
    ret_code = os.system(''.join([script_dir , os.sep, '../classification/PmSVM', os.sep, 'pmsvm',        # executable    - classifier
    ...
    logging.debug(''.join(['\t\tPmSVM classification time: ', str("%0.3f" % (time.time()-start_time)), ' seconds.']))
    if ret_code != 0:
...
#        sys.exit(1)
    ...
    ret_code = os.system(''.join(['g++ -O3 ',
                            script_dir, os.sep, '../classification/PmSVM', os.sep, 'PmSVM.cpp ',
                            '-o ', script_dir, os.sep, '../classification/PmSVM', os.sep, 'pmsvm']))
    if ret_code != 0:
    ...
    for run in range(1, args.repetitions+1):
        ...
        input_run_dir = ''.join([args.source_dir_data, os.sep,'run_', str(run).zfill(3)])
        ...
        #sklearn.externals.joblib.dump(tweets_sampled, ''.join([run_dir, os.sep, 'tweets_sampled.pkl']))
        #continue

        #tweets_sampled = sklearn.externals.joblib.load(''.join([run_dir, os.sep, 'tweets_sampled.pkl']))
        logging.debug(input_run_dir)
        tweets_sampled = sample_tweets(input_run_dir, args.features)

        fold_accuracy_accumulator = 0.0
        logging.debug('\tFolding the dataset ...')
        tweets_per_author = len(tweets_sampled[tweets_sampled.keys()[0]])
        folds = sklearn.cross_validation.KFold(tweets_per_author, n_folds=args.validation_folding)
        fold_id = 0
        for train, test in folds:
            ...
            logging.debug(''.join(['\t\tFeature vector creation time: ', str("%0.3f" % (time.time()-start_time)), ' seconds.']))
