...
class data_provider(object):
    def __init__(self, number_of_users = 20, sentence_max_len = 30, number_of_post_per_user = 2000):
        ...
        if self.number_of_users > len(lst_username):
        ...
        user_selection_idx = np.random.permutation([x for x in range(len(lst_username))])[:number_of_users]
        ...
        for twitter_username in lst_twitter_username:
            ...
            for i in range(len(temp_set)):
                if random_set[i] == 1:
                    ...
                    label = [0 for x in range(number_of_users)]
                    label[k] = 1
            ...
            with open('../tumblr_twitter_scrapper/merged_posts/{0}.csv'.format(tumblr_username), 'r', encoding='utf-8') as f:
                ...
                for row in reader:
                    ...
                    if item.is_owner:
                        ...
                        if(content_count < 3):
                            continue
                        if content_count > sentence_max_len:
        ...
        for i in range(len(train_data)):
            ...
            temp = self.pad_word_ids(self.text_to_word_ids(txt, word_2_id), sentence_max_len)
            if np.sum(temp) > 0:
    ...
    def get_next_test_batch(self):
        label = [0 for x in range(self.number_of_users)]
        label[self.test_batch_counter] = 1
        indices = [label == i for i in self.y_test_set]
        batch_size = len(indices)

        if(self.test_batch_counter == self.number_of_users):
            self.test_batch_counter = 0
