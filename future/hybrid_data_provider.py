...
class data_provider(object):
    def __init__(self, size = 10, sent_max_len = 30, sent_max_char_len = 500, number_of_post_per_user = 50):
        ...
        char_2_id = self.build_char_vocab(' '.join(train_data))

        self.vocab_size = len(word_2_id) + 1
        self.vocab_char_size = len(char_2_id) + 1
        ...
        for i in range(len(train_data)):
            ...
            temp_char = self.pad_char_ids(self.text_to_char_ids(txt, char_2_id), sent_max_char_len)

            if np.sum(temp) > 0:
                self.train_set.append(temp)
                self.y_train_set.append(y_train_data[i])
                self.train_char_set.append(temp_char)
        ...
        for i in range(len(valid_data)):
            ...
            if np.sum(temp) > 0:
                ...
                self.valid_char_set.append(temp_char)
        ...
        for i in range(len(test_data)):
            ...
            if np.sum(temp) > 0:
                ...
                self.test_char_set.append(temp_char)
    ...
    def build_char_vocab(self, data):
        chars = set(data)
        char_to_id = dict(zip(chars, range(1, len(chars)+1)))

        return char_to_id

    
    def text_to_char_ids(self, data, char_to_id):
        return [char_to_id[c] if c in char_to_id else 0 for c in data]
    def text_to_word_ids(self,data, word_to_id):
        #data = _read_words(filename)
        return [word_to_id[word] if word in word_to_id else 0 for word in data.split(' ')]
    ...
    def pad_char_ids(self, char_ids, max_length):
        data_len = len(char_ids)

        if data_len < max_length:
            char_ids = np.lib.pad(char_ids, (0, max_length - data_len), 'constant').tolist()
        
        return char_ids[:max_length]