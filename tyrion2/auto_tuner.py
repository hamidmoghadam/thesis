...
for test_no in range(30) :
    ...
    with open('tune/test_{0}_{1}_{2}_{3}_{4}_{5}.txt'.format(test_no,config['word_h'], config['w_embedding'], config['letter_h'], config['l_embedding'], config['dropout'].repalce('.', '-')), 'w', encoding='utf-8') as f:
        for it in range(30):
