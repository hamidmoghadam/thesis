...
# Define weights
weights = {
    'out': tf.Variable(tf.random_normal([2*n_word_hidden + 3*n_letter_hidden, n_classes]))
}
biases = {
...
def RNN(x, z, weights, biases, dropout, is_training):
    ...
    with tf.variable_scope("letter"):
        u = tf.reshape(z, shape=[-1, n_sent_letters, n_letter_embedding, 1])
    ...
    with tf.variable_scope("letter2"):
        a = tf.cond(tf.equal(is_training, tf.constant(True)), lambda: tf.nn.dropout(z, dropout), lambda:z)
        a = tf.unstack(a, n_sent_letters, 1)
        ...
        outputs_letter, _, _ = rnn.static_bidirectional_rnn(fw_letter_lstm_cell, bw_letter_lstm_cell, a, dtype=tf.float32)
    ...
    final_output = tf.concat((max_output, output_letter, max_output_letter), 1)
