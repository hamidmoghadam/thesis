...
# Define weights
weights = {
    'out': tf.Variable(tf.random_normal([2*n_word_hidden + 2*n_letter_hidden, n_classes]))
}
biases = {
...
def RNN(x, u, weights, biases, dropout, is_training):
    ...
    with tf.variable_scope("word"):
        ...
        fw_lstm_cell = rnn.BasicLSTMCell(n_word_hidden , forget_bias=1.0)
        bw_lstm_cell = rnn.BasicLSTMCell(n_word_hidden, forget_bias=1.0)

        outputs, states, _= rnn.static_bidirectional_rnn(fw_lstm_cell, bw_lstm_cell, x, dtype=tf.float32)

    with tf.variable_scope("letter"):
        u = tf.cond(tf.equal(is_training, tf.constant(True)), lambda: tf.nn.dropout(u, dropout), lambda:u)
        u = tf.unstack(u, n_sent_letters, 1)
        fw_letter_lstm_cell = rnn.BasicLSTMCell(n_letter_hidden , forget_bias=1.0)
        bw_letter_lstm_cell = rnn.BasicLSTMCell(n_letter_hidden , forget_bias=1.0)
        outputs_letter, states_letter, b = rnn.static_bidirectional_rnn(fw_letter_lstm_cell, bw_letter_lstm_cell, u, dtype=tf.float32)
    ...
    max_output_letter = outputs_letter[0]
    
    for i in range(1, len(outputs_letter)):
        max_output_letter = tf.maximum(max_output_letter, outputs_letter[i])

    final_output = tf.concat((max_output, max_output_letter), 1)
